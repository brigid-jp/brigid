...
namespace brigid {
  ...
  void check_impl(bool null_pointer) {
    ...
    if (!clazz) {
      throw BRIGID_RUNTIME_ERROR("cannot FindClass");
    }
    ...
    if (!method) {
      throw BRIGID_RUNTIME_ERROR("cannot GetMethodID");
    }
    ...
    if (!result) {
      throw BRIGID_RUNTIME_ERROR("cannot CallObjectMethod");
    }
    ...
    if (env->ExceptionCheck()) {
      env->ExceptionClear();
      throw BRIGID_RUNTIME_ERROR("cannot GetStringUTFRegion");
    }
